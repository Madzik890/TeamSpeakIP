...
const char* ts3plugin_name() {
	...
	if (!result) {
		const wchar_t* name = L"TeamSpeakIP";
		if (wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			return "TeamSpeakIP";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
...
const char* ts3plugin_author() {
	...
	return "Marcin Sucharski";
}

/* Plugin description */
const char* ts3plugin_description() {
	/* If you want to use wchar_t, see ts3plugin_name() on how to use */
	return "This plugin copy IP server into clipboard";
}
...
void CALLBACK copyierIP(HWND hwnd, UINT uMsg, UINT timerId, DWORD dwTime)
{
	if (GetKeyState(VK_LCONTROL) < 0 && GetKeyState(VK_END) < 0)
		toClipboard(server_ip);
}
...
int ts3plugin_init() {
	...
	SetTimer(NULL, 0, 100, (TIMERPROC)&copyierIP); 

	return 0;  /* 0 = success, 1 = failure, -2 = failure but client will not show a "failed to load" warning */
			   /* -2 is a very special case and should only be used if a plugin displays a dialog (e.g. overlay) asking the user to disable
			   * the plugin again, avoiding the show another dialogby the client telling the user the plugin failed to load.
...
void ts3plugin_onConnectStatusChangeEvent(uint64 serverConnectionHandlerID, int newStatus, unsigned int errorNumber) {
	...
	if (newStatus == STATUS_CONNECTION_ESTABLISHED) {  /* connection established and we have client and channels available */
		unsigned int error_ip,error_port;
		char* ip = NULL;
		uint64 port = NULL;

		anyID clientID = 0;
		ts3Functions.getClientID(serverConnectionHandlerID, &clientID);
		error_ip = ts3Functions.getConnectionVariableAsString(serverConnectionHandlerID, clientID, CONNECTION_SERVER_IP, &ip);
		error_port = ts3Functions.getConnectionVariableAsUInt64(serverConnectionHandlerID, clientID, CONNECTION_SERVER_PORT, &port);
	
		if (error_ip == ERROR_ok || error_port == ERROR_ok)
		{
			server_ip = ip;
			server_ip += ":";
			std::string * sport = new std::string();
			uint64_to_string(port, *sport);
			server_ip += *sport;
			server_ip += port;
			...
			ts3Functions.logMessage("Cannot get Ip", LogLevel::LogLevel_ERROR, "TeamSpeakIP", 0);
